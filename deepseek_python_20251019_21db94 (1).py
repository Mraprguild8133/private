...
**Note:** Most commands require admin privileges.
    ...
    async def demote_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.error(f"Telegram error in demote_command: {e}")
            await update.message.reply_text("❌ Failed to demote user. Make sure I have admin rights.")
        except Exception as e:
            logger.error(f"Error in demote_command: {e}")
            await self.send_error_message(update, "Failed to demote user")
    ...
    async def unban_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            target_user = await self.get_user_from_message(update, context)
            if not target_user:
                await update.message.reply_text("❌ Please specify a user to unban. Usage: /unban @username")
                return

            chat_id = update.effective_chat.id
            await context.bot.unban_chat_member(chat_id=chat_id, user_id=target_user.id)
            await update.message.reply_text(f"✅ Successfully unbanned {target_user.first_name}!")
            
        except TelegramError as e:
            logger.error(f"Telegram error in unban_command: {e}")
            await update.message.reply_text("❌ Failed to unban user.")
        except Exception as e:
            logger.error(f"Error in unban_command: {e}")
            await self.send_error_message(update, "Failed to unban user")
    ...
    async def unmute_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            target_user = await self.get_user_from_message(update, context)
            if not target_user:
                await update.message.reply_text("❌ Please specify a user to unmute. Usage: /unmute @username")
                return
            ...
            logger.error(f"Telegram error in unmute_command: {e}")
            await update.message.reply_text("❌ Failed to unmute user.")
        except Exception as e:
            logger.error(f"Error in unmute_command: {e}")
            await self.send_error_message(update, "Failed to unmute user")
...
Total Warnings: {warning_count}/{config.MAX_WARNINGS}
            ...
            await self.send_error_message(update, "Failed to check warnings")
    ...
    async def pin_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.error(f"Telegram error in pin_message: {e}")
            await update.message.reply_text("❌ Failed to pin message.")
        except Exception as e:
            logger.error(f"Error in pin_message: {e}")
            await self.send_error_message(update, "Failed to pin message")

    async def unpin_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Unpin a message."""
        try:
            if not await self.is_user_admin(update, context):
                await update.message.reply_text("❌ You need to be an admin to use this command.")
            ...
            logger.error(f"Telegram error in unpin_message: {e}")
            await update.message.reply_text("❌ Failed to unpin message.")
        except Exception as e:
            logger.error(f"Error in unpin_message: {e}")
            await self.send_error_message(update, "Failed to unpin message")
    ...
    async def settings_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton("🛡 Auto Moderation", callback_data="auto_mod"),
                    InlineKeyboardButton("👋 Welcome Message", callback_data="welcome_msg")
                ],
                [
                    InlineKeyboardButton("🔗 Link Filter", callback_data="link_filter"),
            ...
            await update.message.reply_text(
                "⚙️ **Bot Settings Panel**\n\nConfigure your group management settings:",
                reply_markup=reply_markup,
                parse_mode='Markdown'
            )
            ...
            await self.send_error_message(update, "Failed to show settings")
...
📝 Type: {chat.type}
            ...
            await self.send_error_message(update, "Failed to get statistics")
    ...
    async def broadcast_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(f"📢 Broadcast message: {broadcast_message}\n\n(Note: In full implementation, this would be sent to all groups)")
            
        except Exception as e:
            logger.error(f"Error in broadcast_command: {e}")
            await self.send_error_message(update, "Failed to process broadcast")

    async def left_member_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle member leaves."""
        try:
            if not config.GOODBYE_MESSAGE_ENABLED:
                return
            ...
            logger.error(f"Error in left_member_handler: {e}")
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            button_handlers = {
                ...
                "auto_mod": "🛡 **Auto Moderation**\n\nAuto moderation is currently enabled.",
                "welcome_msg": "👋 **Welcome Messages**\n\nWelcome messages are currently enabled.",
                "link_filter": "🔗 **Link Filter**\n\nLink filtering is currently enabled.",
                "bot_settings": "🤖 **Bot Settings**\n\nUse environment variables to configure bot settings."
            }
